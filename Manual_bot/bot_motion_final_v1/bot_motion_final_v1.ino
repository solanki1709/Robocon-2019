...
void setup() 
{
  ...
  pwm1_init();
  pwm3_init();
  pinMode(pistonA,OUTPUT);  
  pinMode(pistonB,OUTPUT);
}
...
void bot_motion()
{
    ...
    else if(mright_X < 0 && mleft_Y ==0)    //left
    {
    ...
    else if(mleft_Y >0 && mright_X == 0)   //forward
    { 
    ...
    else if(mleft_Y <0 && mright_X == 0)   //backward
    {
    ...
    else if(jstick.Button(PSB_R2))    //spot left
    {
      setPWM(spot_pwm);
      spot_right();
    ...
    else if(jstick.Button(PSB_L2))    //spot right
    {
      setPWM(spot_pwm);
      spot_left();
    ...
    else if(mleft_Y==0 && mright_X==0)   //bot stop
    {
      bot_pwm=0;
      bot_stop(); 
      Serial.println("No PWM"); 
    }
    if(jstick.ButtonPressed(PSB_CROSS))  //Gerege Transfer
    {
      digitalWrite(pistonA, HIGH);
      digitalWrite(pistonB, LOW);
      }
    if(jstick.ButtonPressed(PSB_CIRCLE))  //Gerege Transfer
    {
      digitalWrite(pistonA, LOW);
      digitalWrite(pistonB, LOW);
    }
}
