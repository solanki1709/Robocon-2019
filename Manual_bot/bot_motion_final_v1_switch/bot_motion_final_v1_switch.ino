...
void readVal()
{
  ...
  // center x 127
  // center y 128
  // origin of stick at left top corner
  //  right_X = map(right_X,0,255,-127,128);
  //  mleft_Y = map(left_Y,0,255,128,-127);
}
void bot_motion()
{
  
  if (left_Y != 128 || right_X != 127)
  {
    if (jstick.Button(PSB_R1))
      bot_pwm = 666;
    else if (left_Y!=128)
      bot_pwm = map(abs(128-left_Y), 0, 128, 200, 500);
    else if (right_X!=127)
      bot_pwm = map(abs(128-right_X), 0, 128, 200, 500);
    setPWM(bot_pwm);
    ...
    switch (right_X / 127)
    {
      case 0: bot_forward();
              ...
              Serial.println(bot_pwm);
              break;
      case 1: bot_pwm = map(right_X, 0, -127, 200, 500);
              setPWM(bot_pwm);
              bot_backward();
              Serial.print("pwm_Back is ");
              Serial.println(bot_pwm);
  ...
  if (right_X > 0 && left_Y == 0)
  {
    bot_pwm = map(right_X, 0, 128, 200, 500);
    if (jstick.Button(PSB_R1))
      bot_pwm = 666;
    setPWM(bot_pwm);
    bot_right();
  ...
  else if (right_X < 0 && left_Y == 0)
  {
    bot_pwm = map(right_X, 0, -127, 200, 500);
    if (jstick.Button(PSB_R1))
      bot_pwm = 666;
    setPWM(bot_pwm);
    bot_left();
  ...
  else if (left_Y > 0 && right_X == 0)
  {
    bot_pwm = map(left_Y, 0, 128, 200, 500);
    if (jstick.Button(PSB_R1))
      bot_pwm = 666;
    setPWM(bot_pwm);
    bot_forward();
  ...
  {
    bot_pwm = map(left_Y, 0, -127, 200, 500);
    if (jstick.Button(PSB_R1))
      bot_pwm = 666;
    setPWM(bot_pwm);
    bot_backward();
