...
d=(-1)*(a*a+b*b+a2*a2-a3*a3)/(2*a2)  

             #confirm a1 or a3	
if(d==0 and (r*r-d*d)==0):
	t2phi=0
elif((r*r-d*d)==0 and d<0 and y!=0):								##removed all 8 round offs for r2-d2 and sqrt
	t2phi= -np.pi/2              
elif((r*r-d*d)==0 and d>0 and y!=0):
	t2phi= +np.pi/2                       #check +-
elif((r*r-d*d)==0 and d<0 and y==0):
	t2phi= -np.pi/2              
elif((r*r-d*d)==0 and d>0 and y==0):
...
#t2phi=np.arctan2(d,round(np.sqrt(r*r-d*d)))
	...
	t2phi=(-1)*np.arctan2(d,(np.sqrt(np.absolute(r*r-d*d))))
elif(y<0):
		t2phi=np.arctan2(d,(np.sqrt(np.absolute(r*r-d*d))))
elif(z>1):
			t2phi=np.arctan2(d,(np.sqrt(np.absolute(r*r-d*d))))
elif(z<1):
		t2phi=(-1)*np.arctan2(d,(np.sqrt(np.absolue(r*r-d*d))))
